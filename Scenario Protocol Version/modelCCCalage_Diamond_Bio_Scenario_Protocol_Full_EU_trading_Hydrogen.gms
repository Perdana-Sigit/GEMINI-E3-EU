...
Ghg Non CO2 GHG  /CH4_RIC,CH4_AGR,CH4_AWB,CH4_FOB,CH4_GRB,CH4_PEB,CH4_EHP_COA,CH4_EHP_CRU,CH4_EHP_OIL,CH4_EHP_GAS,CH4_EHP_BIO,CH4_FPT_COA,CH4_FPT_CRU,CH4_FPT_OIL,CH4_FPT_GAS,CH4_OGF,CH4_IND_COA,CH4_IND_CRU,CH4_IND_OIL,CH4_IND_GAS
                  CH4_NRT_COA,CH4_NRT_CRU,CH4_NRT_OIL,CH4_NRT_GAS,CH4_ROT_COA,CH4_ROT_CRU,CH4_ROT_OIL,CH4_ROT_GAS,CH4_IPP,CH4_WAS,CH4_OTH_COA,CH4_OTH_CRU,CH4_OTH_OIL,CH4_OTH_GAS,CH4_OTH_BIO,CH4_COM_COA,CH4_COM_CRU
                  CH4_COM_OIL,CH4_COM_BIO,
                  CH4_COM_GAS,CH4_RES_COA,CH4_RES_CRU,CH4_RES_OIL,CH4_RES_GAS,CH4_RES_BIO
                  N2O_CRO,N2O_LIV,N2O_FCA_COA,N2O_FCA_CRU,N2O_FCA_OIL,N2O_FCA_GAS,N2O_FCA_BIO,N2O_IPP,N2O_WAS,N2O_LAN,N2O_FUG,N2O_FCH_COA,N2O_FCH_CRU,N2O_FCH_OIL,N2O_FCH_GAS,N2O_FCH_BIO
                  SF6_MAG,SF6_EPS,SF6_FPD,SF6_SEM
...
Secm6(sec)   sector minus hydrogen /01,02,03,04,05,07,08,09,10,11,12,13/
...
PCOAL(T,reg)          exogenous price of coal
POIL(T,reg)           exogenous price of oil
PGAS(T,reg)           exogenous price of natural gas
...
ACXD(ghg,sec,reg)      Coefficient for ghg emissions
AELECGEN(ghg,elec,reg) Coefficient for ghg emissions
ACIO(ghg,pro,sec,reg)  Coefficient for ghg emissions
ACHCV(ghg,sec,reg)     Coefficient for ghg emissions
...
EUETSPRICE2030      Price of EU ETS in 2030
CO2QEUETS           Constraint on EU ETS market
*CO2QEUESR(regeu)    Constraint on EU ESR market
CO2QEUESR(reg)      Constraint on EU ESR market
CO2QNZEEU           NZE target for EU
CO2QCHIETS          Constraint on Chinese ETS market
CO2Q(reg)           CO2 limit
CCSSHARE(elec,reg)  share of CCS
CCSSHARE_CO2_IPP(reg)   share of CCS in CO2 IPP
CCSGROWTH_CO2_IPP(reg)  parameter for CCS in CO2_IPP
...
HYDROGENPJ0(T,sec,reg)   Hydrogen
ELECGEN1(elec,reg)       Generation of power plant ELE in TWh
GENFOSS1(reg)            Electricity generation from fossil fuel power plant
PGENMHYDNUC1(reg)        price of total electricity generation - hydro and nuclear electricity generation
SHAREEVT(sec,reg,T)     share of electric vehicle by sector

;

Variable
EUETSPRICE2             EU ETS 2 price
...
TAXECO2ESR(reg)         Carbon tax
TAXECO2NEU(reg)         Carbon tax non EU countries
...
EUETSPRICE,EUETSPRICE2,TAXECO2,TAXECO2ESR,TAXECO2NEU,PK_06
;
...
MREUETS2                 Market clearing EU ETS 2 market
MREUESR(regeu)           Market clearing EU ESR market
MRCHIETS                 Market clearing EU ETS market
MRNZEEU                  Market Clearing NZE target at EU
ETAXECO2(reg)            CO2 tax
ETAXECO2B(reg)           CO2 tax
ETAXECO2NZE(reg)         CO2 tax
MRCO2(reg)               Constraint on emissions
MREUETS2                 Constraint on EU emissions
MRGHG(regmeu)            Market clearing non EU countries
ETAXECO2(reg)            CO2 tax
EQUOTANETB(reg)          GHG Quota exchange = 0
EQUOTANET(reg)           GHG Quota exchange
EPK_06(reg)              Equation computing PK for Hydrogen
EKAV_06(reg)             Equation computing PK for Hydrogen
...
$LOAD ACXDT AELECGENT ACIOT AHCVT GWP

loop(T,
if ((ord(T) gt 33) and (ord(T) le 41),
 ACXDT(T,"CO2_IPP",secff,"IND")=ACXDT(T-1,"CO2_IPP",secff,"IND")*0.90;
 testind(T)= ACXDT(T,"CO2_IPP","07","IND");

)
if (ord(T) gt 41,
 ACXDT(T,"CO2_IPP",secff,"IND")=ACXDT(T-1,"CO2_IPP",secff,"IND")*0.95;
 testind(T)= ACXDT(T,"CO2_IPP","07","IND");
...
$include Data_scenario_Protocol
...
$LOAD DECLELECT ALELECPD growth0 MAXPOTENTIAL KMPERTOE ALCAPACITY SHARECAREV SHARECARBIO TGCV POP CCSGROWTH CURRENTPOLICIES FABAT SHAREEVT
$GDXIN
...
$LOAD GENMHYDNUCM20 GENMHYDNUCM10 PEXB PHCB PGCB PINVB PBB PIMPB TRANAE0 TRAP0 TRAO0 ENMAPOWER0 HYDROGENPJ0
$GDXIN
...
EPD(sec,reg)..       PD(sec,reg) =E= (APD(sec,reg)*(
                                     ...
                                     **(1/(1-SIGMA(sec,reg))))$(XD0(sec,reg) ne 0)+1$((XD0(sec,reg) eq 0) and (ord(sec) ne 6))+

                                     (APD("06",reg)*(
                                     DK("06",reg)*(PK_06(reg)/TGK("06",reg))**(1-SIGMA("06",reg))+
                                     DL("06",reg)*(PL("06",reg)/TGL("06",reg))**(1-SIGMA("06",reg))+
                                     DE("06",reg)*(PE("06",reg)/TGE("06",reg))**(1-SIGMA("06",reg))+
                                     DM("06",reg)*(PM("06",reg)/TGM("06",reg))**(1-SIGMA("06",reg)))
                                     **(1/(1-SIGMA("06",reg))))$(ord(sec) eq 6)
 ;




EKAV(secm6,reg)..     KAV(secm6,reg)*TGK(secm6,reg) =E=  (XD(secm6,reg)*APD(secm6,reg)*DK(secm6,reg)*
                                      (PD(secm6,reg)/((PK(secm6,reg)*APD(secm6,reg))/(TGK(secm6,reg))))
                                      **SIGMA(secm6,reg))$(KAV0(secm6,reg) ne 0)+0;
...
EPIO(pro,sec,reg)..  PIO(pro,sec,reg) =E=  PB(pro,reg)*(1+TXTVAIO(pro,sec,reg))
                                           ...
                                           +(DTXTVAIO(pro,sec,reg)*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg)))$((ord(reg) le 14) and ((ord(sec) eq 4) or (ord(sec) eq 5) or (ord(sec) eq 6) or (ord(sec) eq 8)))
                                           +(DTXTVAIO(pro,sec,reg)*(SHARE_ETS_AVIATION(reg)*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg))+(1-SHARE_ETS_AVIATION(reg))*TAXECO2(reg)))$((ord(reg) le 14) and (ord(sec) eq 12))
...
EPELEC(elec,reg)..                     PELEC(elec,reg) =E= ((1-TSUB(elec,reg))*APELEC(elec,reg)*(
                                                     ...
                                                     -(CARBONCREDIT(elec,reg)*CCSSHARE(elec,reg)*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg)))$(ord(reg) le 14)
                                                     -(CARBONCREDIT(elec,reg)*CCSSHARE(elec,reg)*TAXECO2(reg))$(ord(reg) gt 14)
...
EPELECENMA(elec,reg)..  PELECENMA(elec,reg) =E= sum(pro,DEPRPOWER(elec,pro,reg)*(PB(pro,reg)*(1+TXTVAIOPOWER(elec,pro,reg))+((1-CCSSHARE(elec,reg))*DTXTVAIOPOWER(elec,pro,reg)*TAXECO2(reg))$(ord(reg) gt 14)
                                                   +((1-CCSSHARE(elec,reg))*DTXTVAIOPOWER(elec,pro,reg)*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg)))$(ord(reg) le 14)));
...
EFSEFA2(reg)..          FSEFA_H_K(reg) =E= sum(secm6,KAVC(secm6,reg)*PK(secm6,reg))+sum(elec,PELECKAV(elec,reg)*ELECKAVC(elec,reg))+KAVC("06",reg)*PK_06(reg);
...
EFGRS5(reg)..           FGRS_VATIO_G(reg)  =E= sum(pro,sum(sec,IOV(pro,sec,reg)*TXTVAIO(pro,sec,reg)*PB(pro,reg)))+
                                               ...
                                               (sum(secem7,sum(secets,IOV(secem7,secets,reg)*DTXTVAIO(secem7,secets,reg)))*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg)))$(ord(reg) le 14)+
                                               (sum(secem7,IOV(secem7,"12",reg)*DTXTVAIO(secem7,"12",reg))*(SHARE_ETS_AVIATION(reg)*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg))+(1-SHARE_ETS_AVIATION(reg))*TAXECO2(reg)))$(ord(reg) le 14)+
                                               (sum(secem7,sum(secets,IOV(secem7,secets,reg)*DTXTVAIO(secem7,secets,reg)))*TAXECO2(reg))$(ord(reg) gt 14)+
                                               (sum(secem7,sum(elec,DEPRPOWER(elec,secem7,reg)*ENMAPOWER0(elec,reg)*DTXTVAIOPOWER(elec,secem7,reg)*(1-CCSSHARE(elec,reg))))*TAXECO2(reg))$(ord(reg) gt 14)+
                                               (sum(secem7,sum(elec,DEPRPOWER(elec,secem7,reg)*ENMAPOWER0(elec,reg)*DTXTVAIOPOWER(elec,secem7,reg)*(1-CCSSHARE(elec,reg))))*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg)))$(ord(reg) le 14)
                                               -sum(elec,(PELEC(elec,reg)/(1-TSUB(elec,reg)))*TSUB(elec,reg)*ELECGEN(elec,reg))
                                               -(CARBONCREDIT("BIO",reg)*CCSSHARE("BIO",reg)*MAX(EUETSPRICE2030,EUETSPRICE)*(EX("USA")/EX(reg))*ELECGEN("BIO",reg))$(ord(reg) le 14)
                                               -(CARBONCREDIT("BIO",reg)*CCSSHARE("BIO",reg)*TAXECO2(reg)*ELECGEN("BIO",reg))$(ord(reg) gt 14);
...
MRKK(sec,reg)$((KAVC0(sec,reg) ne 0) or (ord(sec) ne 6))..          KAV(sec,reg)     =E= KAVC(sec,reg);

MRELECKAV(elec,reg)..   ELECKAVC(elec,reg) =E= ELECKAV(elec,reg);
...
EKAV_06(reg)..     KAVC("06",reg)*TGK("06",reg) =E=  (XD("06",reg)*APD("06",reg)*DK("06",reg)*
                                      (PD("06",reg)/((PK_06(reg)*APD("06",reg))/(TGK("06",reg))))
                                      **SIGMA("06",reg))$(KAV0("06",reg) ne 0)+0;

EPK_06(reg)..    PK_06(reg) =E= PK0("06",reg);
...
MRGHG(regmeu)..    CO2Q(regmeu) =G=       sum(secef,sum(elec,(1-CCSSHARE(elec,regmeu))*CTEPOWER(elec,secef,regmeu)*DEPRPOWER(elec,secef,regmeu)*ENMAPOWER(elec,regmeu)*CO2CONTENTIOPOWER(elec,secef,regmeu)))+
                                         ...
                                         -EGHG("CH4_GRB",regmeu)*GWP("CH4_GRB")
                                         -EGHG("CH4_AWB",regmeu)*GWP("CH4_AWB")
...
EEGHG(ghg,reg)..    EGHG(ghg,reg)=E=(
                                    ...
                                    sum(elec,(1-ABAT(ghg,reg))*AELECGEN(ghg,elec,reg)*ELECGEN(elec,reg))
                                    )/1000;
...
MREUETS2..         sum(regeu,CO2Q(regeu)) =G= sum(regeu,sum(secef,sum(elec,(1-CCSSHARE(elec,regeu))*CTEPOWER(elec,secef,regeu)*DEPRPOWER(elec,secef,regeu)*ENMAPOWER(elec,regeu)*CO2CONTENTIOPOWER(elec,secef,regeu)))+
                                         sum(sec,sum(secef,CTEP(secef,sec,regeu)*IOV(secef,sec,regeu)*CO2CONTENTIO(secef,sec,regeu)))+
                                         ...
                                         -EGHG("CH4_GRB",regeu)*GWP("CH4_GRB")
                                         -EGHG("CH4_AWB",regeu)*GWP("CH4_AWB")
                                         -CCSSHARE("BIO",regeu)*ELECGEN("BIO",regeu)*CARBONCREDIT("BIO",regeu)/1000);


EQUOTANET(reg)..   QUOTANET(reg)=E= (CO2Q(reg)-
                                    (sum(secef,sum(elec,(1-CCSSHARE(elec,reg))*CTEPOWER(elec,secef,reg)*DEPRPOWER(elec,secef,reg)*ENMAPOWER(elec,reg)*CO2CONTENTIOPOWER(elec,secef,reg)))+
                                     sum(sec,sum(secef,CTEP(secef,sec,reg)*IOV(secef,sec,reg)*CO2CONTENTIO(secef,sec,reg)))+
                                     ...
                                     sum(ghg,EGHG(ghg,reg)*GWP(ghg))
                                     -EGHG("CH4_GRB",reg)*GWP("CH4_GRB")
                                     -EGHG("CH4_AWB",reg)*GWP("CH4_AWB")
                                     -CCSSHARE("BIO",reg)*ELECGEN("BIO",reg)*CARBONCREDIT("BIO",reg)/1000))$(ord(reg) le 14)+0;


ETAXECO2(reg).. TAXECO2(reg)=E=(EUETSPRICE*(EX("USA")/EX(reg)))$(ord(reg) le 14)+TAXECO2NEU(reg)$(ord(reg) gt 14);
...
EEUETSPRICE.EUETSPRICE,ETAXECO2B.TAXECO2,EQUOTANETB.QUOTANET,
EPK_06.PK_06
/;
...
MREUETS.EUETSPRICE,ETAXECO2B.TAXECO2,EEGHG.EGHG,EQUOTANETB.QUOTANET
EKAV_06.PK_06/;
...
EENMAPOWER.ENMAPOWER,EPELECENMA.PELECENMA,EPELECR.PELECR,EPELECRA.PELECRA
EPDPGEN.PDPGEN,EXDPGEN.XDPGEN,EINVVELEC.INVVELEC
EEGHG.EGHG,MRGHG.TAXECO2NEU,MREUETS2.EUETSPRICE,EQUOTANET.QUOTANET,ETAXECO2.TAXECO2
EKAV_06.PK_06/;
...
CCSGROWTH(reg,"BIO")=0.07;
CCSGROWTH(reg,"OIP")=0.07/2;
CCSGROWTH(regeu,"BIO")=0.03;
CCSGROWTH_CO2_IPP(reg)=1/20;
...
if (ELECKAV0(elec,reg) eq 0, PELEC.fx(elec,reg)=1 else PELEC.L(elec,reg)=PELEC0(elec,reg);PELEC.LO(elec,reg)=0.00001*PELEC0(elec,reg));
...
TSAVE.fx(reg)       =        TSAVE0(reg)       ;
TXFGRS_H.fx(reg)       =     TXFGRS0_H(reg);
...
PK_06.L(reg)=PK0("06",reg);
PK_06.LO(reg)=PK0("06",reg)*0.00001;

CCSSHARE(elec,reg)=0;
CCSSHARE_CO2_IPP(reg)=0;
...
display TGKEA,AVAILABITY;

PCOAL("2017",reg)=PDPF0("01",reg);
POIL("2017",reg)=PDPF0("02",reg);
PGAS("2017",reg)=PDPF0("03",reg);


$include Climate_Target_scenario_protocol

OILPRICE("2017",reg)=ENERGYPRICE0("2017","OIP","USA");

ENERGYPRICE0(T,"GAP",regeu)=ENERGYPRICE0(T,"GAP","DEU");
ENERGYPRICE0(T,"GAP","GBR")=ENERGYPRICE0(T,"GAP","DEU");
ENERGYPRICE0(T,"GAP","IND")=ENERGYPRICE0(T,"GAP","CHI");
ENERGYPRICE0(T,"GAP","RUS")=ENERGYPRICE0(T,"GAP","DEU");
ENERGYPRICE0(T,"GAP","CSA")=ENERGYPRICE0(T,"GAP","ROW");
ENERGYPRICE0(T,"GAP","MID")=ENERGYPRICE0(T,"GAP","ROW");
ENERGYPRICE0(T,"GAP","AFR")=ENERGYPRICE0(T,"GAP","ROW");
ENERGYPRICE0(T,"GAP","ASI")=ENERGYPRICE0(T,"GAP","CHI");
GASPRICE("2017",reg)=ENERGYPRICE0("2017","GAP",reg);

ENERGYPRICE0(T,"COA",regeu)=ENERGYPRICE0(T,"COA","DEU");
ENERGYPRICE0(T,"COA","GBR")=ENERGYPRICE0(T,"COA","DEU");
ENERGYPRICE0(T,"COA","IND")=ENERGYPRICE0(T,"COA","CHI");
ENERGYPRICE0(T,"COA","RUS")=ENERGYPRICE0(T,"COA","DEU");
ENERGYPRICE0(T,"COA","CSA")=ENERGYPRICE0(T,"COA","ROW");
ENERGYPRICE0(T,"COA","MID")=ENERGYPRICE0(T,"COA","ROW");
ENERGYPRICE0(T,"COA","AFR")=ENERGYPRICE0(T,"COA","ROW");
ENERGYPRICE0(T,"COA","ASI")=ENERGYPRICE0(T,"COA","CHI");
COALPRICE("2017",reg)=ENERGYPRICE0("2017","COA",reg);
...
XDM1("06",reg)    =        XD.L("06",reg)/(1+sum(pro,HYDROGENPJ0("2017",pro,reg))/sum(pro,HYDROGENPJ0("2016",pro,reg)) );
XDM2("06",reg)    =        XDM1("06",reg)/(1+sum(pro,HYDROGENPJ0("2017",pro,reg))/sum(pro,HYDROGENPJ0("2016",pro,reg)) );
KAVCM1("06",reg)  =        KAVC("06",reg)/(1+sum(pro,HYDROGENPJ0("2017",pro,reg))/sum(pro,HYDROGENPJ0("2016",pro,reg)) );
...
TSUB("GAP","IND")=TSUB("GAP","IND")-0.03;
...
TSUB("SOL","CSA")=TSUB("SOL","CSA")+0.06;
TSUB("WIN","CSA")=TSUB("WIN","CSA")+0.03;
...
if (ord(T) le 33,
                Gemini.optfile =1 ;
                CO2QEUETS=CO2QEUETST(T) ;
                SOLVE GEMINI USING MCP;
                put infoiter iter,GEMINI.modelstat,gemini.resusd /;
                if (GEMINI.modelstat ne 1, iter=2101);
    )

*** 41=2030
if (ord(T) gt 33,
                if(ord(T) eq 34, SOLVE GEMINI USING MCP);
*                if(ord(T) eq 35, CO2QEUETS=CO2QEUETST(T-1); CO2Q(reg)=GHG_Target(T-1,reg);   SOLVE GEMINI2031 USING MCP);
                if(ord(T) eq 52, CO2QEUETS=CO2QEUETST(T-1); CO2Q(reg)=GHG_Target(T-1,reg); SOLVE GEMINI2031 USING MCP);
                Gemini2031.optfile =1 ;
                CO2QEUETS=CO2QEUETST(T) ;
                CO2Q(reg)=GHG_Target(T,reg);
                SOLVE GEMINI2031 USING MCP;
                put infoiter iter,GEMINI2031.modelstat,gemini2031.resusd /;
                if (GEMINI2031.modelstat ne 1, iter=2101);
    );
;

Sumelec(reg)=DELEC("WIN",reg)+DELEC("SOL",reg)+DELEC("OTH",reg)+DELEC("BIO",reg)+DELECFOSS(reg);

display Sumelec,DELECFOSS,DELEC,GEN.L,APXDPGEN,APGEN,GENMHYDNUC.L,APELECFOSS,PGENMHYDNUC.L,PELEC.L,ELECGEN.L,PGENFOSS.L,GENFOSS.L;
...
SORTIEMAC("ST05",T,reg)=SORTIEMAC("HT05",T,reg)/(SORTIEMAC("HT05",T,reg)+SORTIEMAC("HT04",T,reg)+SORTIEMAC("HT07",T,reg));
SORTIEMAC("ST04",T,reg)=SORTIEMAC("HT04",T,reg)/(SORTIEMAC("HT05",T,reg)+SORTIEMAC("HT04",T,reg)+SORTIEMAC("HT07",T,reg));
SORTIEMAC("ST07",T,reg)=SORTIEMAC("HT07",T,reg)/(SORTIEMAC("HT05",T,reg)+SORTIEMAC("HT04",T,reg)+SORTIEMAC("HT07",T,reg));
...
SORTIEMAC("ETSP",T,regeu)=MAX(EUETSPRICE2030,EUETSPRICE.L)*(EX.L("USA")/EX.L(regeu))/SORTIEMAC("PGDP",T,regeu);
...
SORTIESEC("XD",T,"06",reg)=XD.L("06",reg);
...
SORTIESEC("PK",T,"06",reg)=PK_06.L(reg)/SORTIEMAC("PGDP",T,reg);
...
SORTIESEC("ENEH",T,sec,reg)=  CTEP("06",sec,reg)*IOV.L("06",sec,reg);
...
SORTIEMAC("GETS",T,regeu)=sum(secets,sum(secef,CTEP(secef,secets,regeu)*IOV.L(secef,secets,regeu)*CO2CONTENTIO(secef,secets,regeu)))+
                                    sum(secef,sum(elec,(1-CCSSHARE(elec,regeu))*CTEPOWER(elec,secef,regeu)*DEPRPOWER(elec,secef,regeu)*ENMAPOWER.L(elec,regeu)*CO2CONTENTIOPOWER(elec,secef,regeu)))+
                                    SHARE_ETS_AVIATION(regeu)*sum(secef,CTEP(secef,"12",regeu)*IOV.L(secef,"12",regeu)*CO2CONTENTIO(secef,"12",regeu))
                                    -CCSSHARE("BIO",regeu)*ELECGEN.L("BIO",regeu)*CARBONCREDIT("BIO",regeu)/1000;

SORTIEMAC("GESR",T,regeu)=(1-SHARE_ETS_AVIATION(regeu))*sum(secef,CTEP(secef,"12",regeu)*IOV.L(secef,"12",regeu)*CO2CONTENTIO(secef,"12",regeu))+
                                         sum(secnets,sum(secef,CTEP(secef,secnets,regeu)*IOV.L(secef,secnets,regeu)*CO2CONTENTIO(secef,secnets,regeu)))+
                                         sum(secef,CTEPH(secef,regeu)*HCV.L(secef,regeu)*CO2CONTENTH(secef,regeu))+
                                         sum(secef,CTEPG(secef,regeu)*GCV(secef,regeu)*CO2CONTENTG(secef,regeu))+
                                         sum(ghg,EGHG.L(ghg,regeu)*GWP(ghg));
...
*** for 2.5 factor see heat_pump_Database.gms
SORTIEMAC("HPUM",T,reg)=2.5*CTEPH("05",reg)*(HCVHEATPUMP.L(reg)*APHCHEATPUMP(reg)*DHEATPUMP_05(reg)*(PHCHEATPUMP.L(reg)/(PHC.L("05",reg)*APHCHEATPUMP(reg)))**SIGMAHEATPUMP(reg));
SORTIEMAC("IPUM",T,reg)=2.5*CTEP("05","13",reg)*(IOVHEATPUMP.L("13",reg)*APIOHEATPUMP("13",reg)*DIOHEATPUMP_05("13",reg)*(PIOHEATPUMP.L("13",reg)/(PIO.L("05","13",reg)*APIOHEATPUMP("13",reg)))**SIGMAIOHEATPUMP("13",reg));
SORTIEMAC("RSHA",T,reg)=((SORTIEELEC("GENE","HYD",T,reg)+SORTIEELEC("GENE","WIN",T,reg)+SORTIEELEC("GENE","SOL",T,reg)+SORTIEELEC("GENE","BIO",T,reg)+SORTIEELEC("GENE","OTH",T,reg))/1000*(3.6)/41.868+
                          sum(secm,SORTIESEC("ENEB",T,secm,reg))+SORTIEMAC("HENB",T,reg)+SORTIEMAC("HPUM",T,reg)+SORTIEMAC("IPUM",T,reg))/
                        (sum(secm,SORTIESEC("ENEC",T,secm,reg)+SORTIESEC("ENEO",T,secm,reg)+SORTIESEC("ENEG",T,secm,reg)+SORTIESEC("ENEP",T,secm,reg)+SORTIESEC("ENEE",T,secm,reg)+SORTIESEC("ENEB",T,secm,reg))+
                         SORTIEMAC("HENC",T,reg)+SORTIEMAC("HENO",T,reg)+SORTIEMAC("HENO",T,reg)+SORTIEMAC("HENG",T,reg)+SORTIEMAC("HENP",T,reg)+SORTIEMAC("HENE",T,reg)+SORTIEMAC("HENB",T,reg));


OILPRICE(T+1,reg)=ENERGYPRICE0(T+1,"OIP","USA")*SORTIEMAC("PGDP",T,"USA")*(EX.L("USA")/EX.L(reg));
GASPRICE(T+1,reg)=ENERGYPRICE0(T+1,"GAP",reg)*SORTIEMAC("PGDP",T,reg);
COALPRICE(T+1,reg)=ENERGYPRICE0(T+1,"COA",reg)*SORTIEMAC("PGDP",T,reg);


POIL(T+1,reg)   =  PDPF0("02",reg)*(OILPRICE(T+1,reg)/OILPRICE("2017",reg))   ;
PCOAL(T+1,reg)  =  PDPF0("01",reg)*(COALPRICE(T+1,reg)/COALPRICE("2017",reg)) ;
PGAS(T+1,reg)   =  PDPF0("03",reg)*(GASPRICE(T+1,reg)/GASPRICE("2017",reg))   ;
...
SORTIEMAC("HFPT",T,reg)=EGHG.L("CH4_FPT_COA",reg)+EGHG.L("CH4_FPT_CRU",reg)+EGHG.L("CH4_FPT_GAS",reg)+EGHG.L("CH4_FPT_OIL",reg);
SORTIEMAC("HOGF",T,reg)=EGHG.L("CH4_OGF",reg);
SORTIEMAC("HIPP",T,reg)=EGHG.L("CH4_IPP",reg);
SORTIEMAC("HROT",T,reg)=EGHG.L("CH4_ROT_COA",reg)+EGHG.L("CH4_ROT_CRU",reg)+EGHG.L("CH4_ROT_GAS",reg)+EGHG.L("CH4_ROT_OIL",reg);
SORTIEMAC("HIND",T,reg)=EGHG.L("CH4_IND_COA",reg)+EGHG.L("CH4_IND_CRU",reg)+EGHG.L("CH4_IND_GAS",reg)+EGHG.L("CH4_IND_OIL",reg);
SORTIEMAC("HRCO",T,reg)=EGHG.L("CH4_OTH_COA",reg)+EGHG.L("CH4_OTH_CRU",reg)+EGHG.L("CH4_OTH_GAS",reg)+EGHG.L("CH4_OTH_OIL",reg)+EGHG.L("CH4_OTH_BIO",reg);
SORTIEMAC("HRET",T,reg)=EGHG.L("CH4_RES_COA",reg)+EGHG.L("CH4_RES_CRU",reg)+EGHG.L("CH4_RES_GAS",reg)+EGHG.L("CH4_RES_OIL",reg)+EGHG.L("CH4_RES_BIO",reg);
...
SORTIEMAC("HNRT",T,reg)=EGHG.L("CH4_NRT_COA",reg)+EGHG.L("CH4_NRT_CRU",reg)+EGHG.L("CH4_NRT_GAS",reg)+EGHG.L("CH4_NRT_OIL",reg);
...
SORTIEMAC("NFCA",T,reg)=EGHG.L("N2O_FCA_COA",reg)+EGHG.L("N2O_FCA_CRU",reg)+EGHG.L("N2O_FCA_GAS",reg)+EGHG.L("N2O_FCA_OIL",reg)+EGHG.L("N2O_FCA_BIO",reg)+
                        EGHG.L("N2O_FCH_COA",reg)+EGHG.L("N2O_FCH_CRU",reg)+EGHG.L("N2O_FCH_GAS",reg)+EGHG.L("N2O_FCH_OIL",reg)+EGHG.L("N2O_FCH_BIO",reg);
...
$include calage_Diamond_bio_hydrogen
$include increase_sigma_diamond
...
if (TAXECO2CCS(reg) gt 90,
CCSSHARE_CO2_IPP(reg)=CCSSHARE_CO2_IPP(reg)+CCSGROWTH_CO2_IPP(reg)*(TAXECO2CCS(reg)/90)**0.15;
   );
);
...
if (iter gt 2030,
loop(regmeu,
if (TAXECO2CCS(regmeu) gt 135,
   CCSSHARE("BIO",regmeu)=CCSSHARE("BIO",regmeu)+CCSGROWTH(regmeu,"BIO")*(TAXECO2CCS(regmeu)/135)**0.15;
);
);
loop(regeu,
if (TAXECO2CCS(regeu) gt 135,
   CCSSHARE("BIO",regeu)=CCSSHARE("BIO",regeu)+CCSGROWTH(regeu,"BIO")*(TAXECO2CCS(regeu)/135)**0.15;
);
);
...
CCSSHARE_CO2_IPP(reg)=MIN(1,CCSSHARE_CO2_IPP(reg));
...
if(CURRENTPOLICIES(reg,"2050","NUC") eq 0,
         ELECKAVCDELTANUC(reg)= ELECKAVC("NUC",reg)/(2050-2024);
         ELECGENDELTANUC(reg)=  ELECGEN.L("NUC",reg)/(2050-2024);
));
);


if(iter eq 2018, ELECKAVCDELTANUC("DEU")=ELECKAVC("NUC","DEU")/(2024-2017); ELECGENDELTANUC("DEU")=ELECGEN.L("NUC","DEU")/(2024-2017));

if(iter le 2024,
    ELECGEN.fx("HYD",reg)=ELECGEN.L("HYD",reg)*(CURRENTPOLICIES(reg,"2024","HYD")/CURRENTPOLICIES(reg,"2017","HYD"))**(1/(2024-2017));
    ELECGEN.fx("NUC",reg)$((ELECGEN.L("NUC",reg) ne 0) and (ord(reg) ne 1))=ELECGEN.L("NUC",reg)*(CURRENTPOLICIES(reg,"2024","NUC")/CURRENTPOLICIES(reg,"2017","NUC"))**(1/(2024-2017));
    ELECGEN.fx("NUC","DEU")=MAX(0,ELECGEN.L("NUC","DEU")-ELECGENDELTANUC("DEU"));

  else
    ELECGEN.fx("HYD",reg)=ELECGEN.L("HYD",reg)*(CURRENTPOLICIES(reg,"2050","HYD")/CURRENTPOLICIES(reg,"2024","HYD"))**(1/(2050-2024));
    ELECGEN.fx("NUC",reg)$((ELECGEN.L("NUC",reg) ne 0) and (CURRENTPOLICIES(reg,"2050","NUC") ne 0))=ELECGEN.L("NUC",reg)*(CURRENTPOLICIES(reg,"2050","NUC")/CURRENTPOLICIES(reg,"2024","NUC"))**(1/(2050-2024));
...
if(iter le 2024,
    ELECKAVO.fx("HYD",reg)=ELECKAVC("HYD",reg)*(CURRENTPOLICIES(reg,"2024","HYD")/CURRENTPOLICIES(reg,"2017","HYD"))**(1/(2024-2017));
    ELECKAVO.fx("NUC",reg)$( (ELECKAVO.L("NUC",reg) ne 0) and (ord(reg) ne 1))=ELECKAVO.L("NUC",reg)*(CURRENTPOLICIES(reg,"2024","NUC")/CURRENTPOLICIES(reg,"2017","NUC"))**(1/(2024-2017));
    ELECKAVO.fx("NUC","DEU")=MAX(0,ELECKAVC("NUC","DEU")-ELECKAVCDELTANUC("DEU"));
  else
    ELECKAVO.fx("HYD",reg)=ELECKAVC("HYD",reg)*(CURRENTPOLICIES(reg,"2050","HYD")/CURRENTPOLICIES(reg,"2024","HYD"))**(1/(2050-2022));
    ELECKAVO.fx("NUC",reg)$((ELECGEN.L("NUC",reg) ne 0) and (CURRENTPOLICIES(reg,"2050","NUC") ne 0))=ELECKAVC("NUC",reg)*(CURRENTPOLICIES(reg,"2050","NUC")/CURRENTPOLICIES(reg,"2024","NUC"))**(1/(2050-2024));
...
ABAT("CO2_IPP",reg)=ABAT("CO2_IPP",reg)*(1-CCSSHARE_CO2_IPP(reg))+CCSSHARE_CO2_IPP(reg);

);


display CCSSHARE_CO2_IPP,ABAT;
...
$include AirPollutants_Modeling



$include IPCC_Report_Reference
